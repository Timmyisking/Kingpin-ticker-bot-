...
def extract_tickers_from_text(text: str) -> list:
    ...
    noise = {
        ...
        "TOO","USE","USD","EUR","GBP","NFT","APR","APY","ATH","ATL","CEO",
        "COO","CFO","LLC","INC","LTD","ETF","IPO","SEC","FBI","CIA","USA",
        "LIVE","NEWS","JUST","LIKE","WITH","THIS","THAT","FROM","THEY",
        "WHAT","WHEN","WILL","YOUR","HAVE","MORE","BEEN","WERE","THAN",
        "THEN","THEM","SOME","INTO","OVER","AFTER","ALSO","BACK","MOST",
        "MADE","MAKE","SAID","EACH","WHICH","THEIR","TIME","WOULD","COULD",
        "SHOULD","ABOUT","TWEET","TIKTOK","VIDEO","POST","LINK","HTTP",
        "HTTPS","HTML","JSON","API","BOT","APP","NEW","TOP","BIG","HOT"
    }
...
def search_dexscreener(ticker: str):
    try:
...
def search_coingecko(ticker: str):
    try:
...
🧛 *Vamps / PVP:* _Not available for this listing_"""
...
async def scan_direct(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(f"🔍 Scanning ${ticker} across all chains...")

    result = await asyncio.get_event_loop().run_in_executor(None, search_dexscreener, ticker)
    if result:
        await msg.edit_text(build_dex_message(ticker, result), parse_mode="Markdown", disable_web_page_preview=True)
        return
    cg = await asyncio.get_event_loop().run_in_executor(None, search_coingecko, ticker)
    if cg:
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not urls:
        ...
        if tickers_in_msg:
            ...
            msg = await update.message.reply_text(f"🔍 Spotted ${ticker} — scanning...")
            result = await asyncio.get_event_loop().run_in_executor(None, search_dexscreener, ticker)
            if result:
                await msg.edit_text(build_dex_message(ticker, result), parse_mode="Markdown", disable_web_page_preview=True)
                return
            ...
            if cg:
                await msg.edit_text(build_cg_message(ticker, cg), parse_mode="Markdown")
                return
            await msg.edit_text(f"❌ *Ticker not found*\n\n`${ticker}` was not found on any chain.", parse_mode="Markdown")
        else:
            await update.message.reply_text(
                "⚠️ Please send a valid link or use /scan TICKER to search directly."
            )
        return

    url = urls[0]
    processing_msg = await update.message.reply_text(f"⏳ Fetching page content...")

    page_text = await asyncio.get_event_loop().run_in_executor(None, fetch_page_text, url)

    if not page_text:
        await processing_msg.edit_text(
            "⚠️ *Could not read that page.*\n\n"
            "Some platforms block bots from reading links directly.\n\n"
            "💡 Try copying the text from the post and pasting it here, "
            "or use /scan TICKER to search directly.",
            parse_mode="Markdown"
        )
        return

    await processing_msg.edit_text("🔍 Extracting tickers from page...")
    tickers = extract_tickers_from_text(page_text)

    if not tickers:
        await processing_msg.edit_text(
            "❌ *Ticker not found*\n\nNo crypto tickers detected on that page.\n\n"
            "💡 Use /scan TICKER to search directly.",
            parse_mode="Markdown"
        )
        return

    preview = " ".join([f"${t}" for t in tickers[:5]])
    await processing_msg.edit_text(f"🔎 Found possible tickers: {preview}\n\nChecking chains...")
    ...
    if not found_results:
        await processing_msg.edit_text(
            "❌ *Ticker not found*\n\n"
            f"Scanned {preview} — none found on\\-chain\\.\n\n"
            "💡 Use /scan TICKER to search directly.",
            parse_mode="Markdown"
        )
        return
    ...
    if source == "dex":
        await processing_msg.edit_text(
            build_dex_message(ticker, data), parse_mode="Markdown", disable_web_page_preview=True
        )
    else:
        await processing_msg.edit_text(build_cg_message(ticker, data), parse_mode="Markdown")
    ...
    for source, ticker, data in found_results[1:]:
        if source == "dex":
            ...
            await update.message.reply_text(build_cg_message(ticker, data), parse_mode="Markdown")
